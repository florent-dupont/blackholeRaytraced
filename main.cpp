        ...
        if (diskCollision)
        {
            ...
            if (diskCollision)
            {
                if (k < rdr.maxtransparency)
                {
                    ...
                    double phi = atan2(coll_y / r, coll_x / r); //Coordonné du point d'impact (en sphérique)
...
{ //arguments: i0 et j0 position du pixel, x,y,z position initiale du rayon dans l'espace
    ...
    //double range = 0.065 * 20. / (rdr.width - 1.0); //FOV
...
void computeChunk(int h, double *image, int *index)
{
    ...
    for (int l = index0; l < index1; ++l)
    {
        ...
        //if ((i < 0.37 * rdr.width + 50) && (i > (0.37 * rdr.width)))
        //{
        ...
        for (int v=0;v<rdr.EntireSamplesPerPixels;v++)
        {
            double tmpR = 0.;
            double tmpG = 0.;
            double tmpB = 0.;
            ...
            double tmpj=j;
            if (v!=0){
                tmpi+=doubleRand(1.)-0.5;
                tmpj+=doubleRand(1.)-0.5;

            }
            

            sim_bundle(tmpi, tmpj, scn.camera.x, scn.camera.y, scn.camera.z, &tmpR, &tmpG, &tmpB); //i horiz, j vert
        ...
        /* pixelR*=rdr.invEntireSamplesPerPixels;
        pixelG*=rdr.invEntireSamplesPerPixels;
        pixelB*=rdr.invEntireSamplesPerPixels;*/
...
{
    cout << "Postprocessing: convolution" << endl;
    ...
    for (int k = 0; k < size; ++k)
    {
        kernel_double[k] = (double)kernel[k] / sum; 
    }
    //

    double *image_convolved = new double[rdr.Npixels * CHANNEL_NUM];

    int Ncomp = CHANNEL_NUM * rdr.Npixels;

    for (int l = 0; l < Ncomp; ++l)
    {
        image_convolved[l] = 0.; //Mettre l'image à zero
    }

    convolution_2D(image, kernel_double, image_convolved, rdr.width, rdr.height, kernel_width, kernel_height);

    char nametmp[50];
    int n;
    n = sprintf(nametmp, "%s_convolved.png", name);
    save(nametmp, image_convolved, rdr.height, rdr.width, CHANNEL_NUM);

    cout << "Postprocessing: bruit" << endl;

    //bruit gaussien
    std::default_random_engine generator;

    std::normal_distribution<double> noise(0.0, .01*255.);

    double min_image=0;

    for (int l = 0; l < Ncomp; ++l)
    {
        image_convolved[l] += noise(generator); 

        if (image_convolved[l]<min_image){
            min_image=image_convolved[l];
        }
    }

    for (int l = 0; l < Ncomp; ++l)
    {
        double tmp=image_convolved[l]-min_image;
        if (tmp>255.){
            image_convolved[l]=255.;
        }else{
            image_convolved[l]=tmp;
        }
    }

    
    n = sprintf(nametmp, "%s_convolved_noise.png", name);
    save(nametmp, image_convolved, rdr.height, rdr.width, CHANNEL_NUM);
...
void render(char *name, double *image)
{
    ...
    pl::async_par_for(0, rdr.TotalChunknumber, [&](unsigned h) { //Le premier est calculé pour estimer le temps de calcul total
        ...
        printf("\rFile %s - Chunk %04d/%04d started - %.2f pourcents", name, h, rdr.TotalChunknumber, pc);
        fflush(stdout);

        computeChunk(h, image, index);
